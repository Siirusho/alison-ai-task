...
const UsersList = ({ users, currentUserId, typingUsers }: ParticipantListProps) => {
    const sortedUsers = useMemo(() => UserHelpers.sortUsersByActivity(users, currentUserId), [users, currentUserId]);
            ...
            <div className="flex-grow p-6 overflow-auto">
                ...
                <ul className="p-0 space-y-0">
                    {sortedUsers.map((user, index) => {
                        ...
                        const statusColor = UserHelpers.getUserStatusColor(user.lastActivity);
                            ...
                            <React.Fragment key={user.id}>
                                <li className="px-1 py-4 flex items-start">
                                    <div className="relative mr-3 flex-shrink-0">
                                        <div
                                            className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-medium ${isCurrentUser ? 'bg-blue-600' : 'bg-gray-500'
                                                }`}
                                        >
                                            {user.username.charAt(0).toUpperCase()}
                                        </div>
                                        <div
                                            className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${isTyping ? 'bg-yellow-500' : statusColorClass
                                                }`}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1.5">
                                            <span className={`text-sm ${isCurrentUser ? 'font-bold' : 'font-normal'} text-gray-900`}>
                                                {user.username}
                                            </span>
                                            {isCurrentUser && (
                                ...
                                {index < sortedUsers.length - 1 && <li className="border-t border-gray-100 ml-12 my-1" />}
                            </React.Fragment>
                        );
                    })}
                </ul>
