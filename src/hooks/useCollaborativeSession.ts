...
import { Participant, Message, CounterState, AppState, SyncMessage, MultiTabSyncHook, SyncEventType } from '../lib/types';
import dayjs from 'dayjs';
import { TimeHelpers } from '../helpers/timeHelpers';
import { UserHelpers } from '../helpers/userHelpers';
import { SESSION_ID, USER_INACTIVE_THRESHOLD, TYPING_TIMEOUT, USER_CLEANUP_INTERVAL, MESSAGE_CLEANUP_INTERVAL } from '../constants/chatConstants';
...
export function useCollaborativeSession(): MultiTabSyncHook {
    ...
    const sendMessage = useCallback((content: string, expiresInSeconds?: number) => {
        ...
        postMessage(SyncEventType.SEND_MESSAGE, { type: SyncEventType.SEND_MESSAGE, payload: message });
        ...
        postMessage(SyncEventType.PARTICIPANT_UPDATE, { type: SyncEventType.PARTICIPANT_UPDATE, payload: updatedUser });
        setUsers(prev =>
    ...
    const deleteMessage = useCallback((messageId: string) => {
        postMessage(SyncEventType.REMOVE_MESSAGE, { type: SyncEventType.REMOVE_MESSAGE, payload: { messageId, userId: userId.current } });
    ...
    const updateCounter = useCallback((increment: boolean) => {
        ...
        postMessage(SyncEventType.UPDATE_COUNTER, { type: SyncEventType.UPDATE_COUNTER, payload: update });
        setCounter({
            value: newValue,
    ...
    const markTyping = useCallback((isTyping: boolean) => {
        if (isTyping) {
            postMessage(SyncEventType.BEGIN_TYPING, { type: SyncEventType.BEGIN_TYPING, payload: { userId: userId.current } });
            ...
            typingTimeoutRef.current = setTimeout(() => {
                postMessage(SyncEventType.END_TYPING, { type: SyncEventType.END_TYPING, payload: { userId: userId.current } });
            }, TYPING_TIMEOUT);
        } else {
            if (typingTimeoutRef.current) {
                clearTimeout(typingTimeoutRef.current);
    ...
    useEffect(() => {
        if (!hasInitialized.current) {
            ...
            postMessage(SyncEventType.PARTICIPANT_JOIN, { type: SyncEventType.PARTICIPANT_JOIN, payload: currentCollaborativeUser });
            postMessage(SyncEventType.REQUEST_SYNC, { type: SyncEventType.REQUEST_SYNC, payload: { requesterId: userId.current } });
        }

        const handleBeforeUnload = () => {
            postMessage(SyncEventType.PARTICIPANT_LEAVE, { type: SyncEventType.PARTICIPANT_LEAVE, payload: { userId: userId.current } });
        };
    ...
    useEffect(() => {
        ...
        messages.forEach((msg: BroadcastChannelMessage) => {
            ...
            if (msg.source === tabId.current &&
                [SyncEventType.SEND_MESSAGE, SyncEventType.UPDATE_COUNTER].includes(action.type)) {
                return;
            }
                            ...
                            setChatMessages(currentMessages => {
                                setCounter(currentCounter => {
                                    setTypingUsers(currentTypingUsers => {
                                        ...
                                        postMessage(SyncEventType.SYNC_STATE, { type: SyncEventType.SYNC_STATE, payload: currentState });
